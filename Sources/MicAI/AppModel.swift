...
final class AppModel: ObservableObject {
  ...
  init() {
    ...
    // One client for commands, refinement, translate and ask, so a provider
    // status seen by one path is the status every path reports.
    let llmClient = CodexCLIClient(
